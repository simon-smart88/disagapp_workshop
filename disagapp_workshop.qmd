...
```{=html}
<object type="image/svg+xml" data="disag_data_types_path.svg"></object>
```
...
- Datasets are either constrained, where satellite imagery has been used to locate dwellings, resulting in areas with zero population

## Upload covariates

- You can also upload your own covariates
- Click *Browse* and select `.tif` files
- They will be converted to the WGS 84 coordinate system and cropped to your data
...
- The first two show the model parameters, on the left hand side are the model parameters, on the right hand side are the covariate parameters
...
- Install the package with `remotes::install_github("simon-smart88/disagapp)` 
...
- These only have 8GB of RAM, so you will need to reduce the covariate resolution to *c.* 5 km<sup>2<sup/> 
